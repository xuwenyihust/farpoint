...
GIT_COMMIT="${12:-${FARPOINT_GIT_COMMIT:-}}"
CONFIG_SHA256="${13:-${FARPOINT_CONFIG_SHA256:-}}"
SIMULATOR_IMAGE_DIGEST="${14:-${FARPOINT_SIMULATOR_IMAGE_DIGEST:-}}"
YAW_PLAN="${15:-${FARPOINT_YAW_PLAN:-}}"
if [[ -n "${POSITION_PLAN}" && "${POSITION_PLAN}" != /* ]]; then
  POSITION_PLAN="/workspace/project/${POSITION_PLAN}"
fi
if [[ -n "${YAW_PLAN}" && "${YAW_PLAN}" != /* ]]; then
  YAW_PLAN="/workspace/project/${YAW_PLAN}"
fi
...
timeout --signal=TERM --kill-after=20 "${RUN_TIMEOUT_SECONDS}" \
  ...
  -e FARPOINT_CONFIG_SHA256="${CONFIG_SHA256}" \
  -e FARPOINT_SIMULATOR_IMAGE="${ISAAC_IMAGE}" \
