#!/usr/bin/env python3
"""Validate historical plus recovery collections and create one selection."""
...
def main() -> int:
    ...
    parser.add_argument(
        "--source-episodes-root", action="append", required=True, type=Path
    )
    ...
    historical_sources = [
        (_read(plan), _read(manifest), episodes_root)
        for plan, manifest, episodes_root in zip(
            args.source_plan,
            args.source_manifest,
            args.source_episodes_root,
        )
    ]
    reference_plan = _read(args.reference_plan)
    ...
    if report["status"] != "PASS":
        print(
            "SO101_MASS_MULTI_SOURCE_COMPLETION_INVALID "
            + ";".join(report["evidence_errors"])
        )
        return 2
    manifest, selection, _report = build_mass_multi_source_completion_selection(
        reference_plan,
        historical_sources,
        recovery_plan,
        recovery_manifest,
    ...
    print(
        f"SO101_MASS_MULTI_SOURCE_COMPLETION_OK "
        f"episodes={len(selection['episodes'])} "
